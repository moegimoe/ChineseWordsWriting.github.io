...
/** 「間違えた問題だけ」モードから除外されるまでに必要な連続正解数 */
const MISTAKE_CLEAR_STREAK = 2;
...
function applyReviewResult(word, isCorrect) {
  ...
  if (isCorrect) {
    ...
    stats.streakSinceMistake = (stats.streakSinceMistake || 0) + 1;
  } else {
    stats.wrong += 1;
    stats.srsLevel = 0;
    stats.streakSinceMistake = 0;
  }
...
/**
 * 「間違えた問題だけ」モードの対象かどうかを判定する。
 * 直近の間違い以降、指定回数連続で正解すると対象から外れる。
 * @param {import('./storage.js').Word} word
 * @returns {boolean}
 */
function isActiveMistake(word) {
  return word.stats.wrong > 0 && (word.stats.streakSinceMistake || 0) < MISTAKE_CLEAR_STREAK;
}
